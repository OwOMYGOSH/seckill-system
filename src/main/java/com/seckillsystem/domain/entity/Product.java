...
@Table(name = "products")
@NamedQueries({
    @NamedQuery(name = "Product.decreaseStock", query = "update Product set stockAvailable = stockAvailable - 1 where id = :id and stockAvailable > 0"),
    @NamedQuery(name = "Product.increaseStock", query = "update Product set stockAvailable = stockAvailable + 1 where id = :id"),
})
